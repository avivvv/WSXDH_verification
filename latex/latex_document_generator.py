from pylatex import Document, Section, NoEscape, NewPage, LineBreak, Package
from latex.latex_table_generator import generate_tex_table
from calculate.sp2n_data_supplier import create_sp2n_data__n_equals


def generate_table_tex_for(n: int):
    tex_filename = f"results/tables__sp{2*n}"
    generate(n, n, tex_filename)


def generate_tables_tex_for_range(min_n: int, max_n: int):
    tex_filename = f"results/tables__sp{2*min_n}-sp{2*max_n}"
